#!/usr/bin/env python3
"""Stop-hook helper: saves the coding-agent chat transcript into the submission's logs/ folder.
...
def _redact_blocks(obj):
    """Strip non-conversation blocks (thinking/tool_use/tool_result) from a kept line.

    Line-level filtering alone is not enough: an assistant turn often carries a
    thinking block and a text block in the SAME content array, so keeping the line
    verbatim would leak the reasoning (which frequently quotes file contents).
    Rebuilds the line with text blocks only. Schema is preserved; only blocks drop.
    """
    for holder_key in ("message", "payload"):
        holder = obj.get(holder_key)
        if isinstance(holder, dict) and isinstance(holder.get("content"), list):
            holder["content"] = [
                b for b in holder["content"]
                if isinstance(b, dict) and b.get("type") == "text"
            ]
    return obj
...
def slim_transcript(raw: str, tool: str):
    """Keep only the conversation text from a transcript, as JSONL.

    Two passes: (1) drop lines that carry no user/assistant conversation text —
    tool calls/results, session metadata, skill listings; (2) within kept lines,
    drop every non-text block so thinking/tool blocks never survive.
    ...
    if not any_parsed:
        return None
    def dump(objs):
        # separators: JSON.stringify(save_log.mjs)와 바이트 단위로 동일한 출력을 내기 위함
        return [json.dumps(_redact_blocks(o), ensure_ascii=False, separators=(",", ":"))
                for o in objs]

    if tool == "codex":
        kept = dump([obj for _, obj in pairs if _codex_has_event_text(obj)])
        if not kept:                            # fallback: response_item (no event_msg present)
            kept = dump([obj for _, obj in pairs if _codex_has_response_text(obj)])
    else:
        kept = dump([obj for _, obj in pairs if _claude_has_text(obj)])
    if not kept:
        return None
    return "\n".join(kept) + "\n"
